...
# ============ FONCTION (compte-rendu structuré) ============
...
def generate_compte_rendu(cleaned_text: str, speakers_summaries: dict = None) -> dict:
    ...
    # Préparer le contexte avec les résumés par speaker
    context_speakers = ""
...
Sois concis, professionnel et factuel. Ne mentionne que ce qui est réellement dit dans la transcription."""
    ...
    try:
        ...
        # Pour le résumé court supp "# CONTEXTE ET OBJECTIF"
        lines = compte_rendu.split('\n')
        resume_lines = []
        in_resume_exec = False
        
        for line in lines:
            if line.strip().startswith("# RÉSUMÉ EXÉCUTIF"):
                in_resume_exec = True
                continue
            elif line.strip().startswith("#") and in_resume_exec:
                # On atteint une nouvelle section, on arrête
                break
            elif in_resume_exec and line.strip():
                resume_lines.append(line)
        
        resume_court = "\n".join(resume_lines).strip()
        
        # Si le résumé court est vide, fallback
        if not resume_court:
            resume_court = compte_rendu[:300]
